#' Calculate Otsu agreement scores using specified threshold for an `mx_dataset` object.
#'
#' @param mx_data `mx_dataset` object used to compute Otsu agreement scores
...
#' @param plot_out boolean to generate Otsu agreement plots (default=FALSE)
#' @param ... optional additional arguments for Otsu agreement functions
#'
...
#'
#' @return `mx_dataset` object with analysis results of Otsu agreement in `otsu_data` table
#'
#' @export
#'
#' @examples
...
run_otsu_agreement <- function(mx_data,
                          table,
                          threshold_override = NULL,
                          plot_out = FALSE,
                          ...){
    ## validate parameters
    mx_obj = validate_otsu_agreement_params(mx_data,
                                            table,
                                            threshold_override)

    ## set correct threshold & validate
    threshold = get_agreement_thold(threshold_override,
                               ...)
    ...
    mx_obj = otsu_mx_agreement(mx_obj,
                                       table)

    ## plot out if desired
    if(plot_out){
        print(plot_mx_density(mx_obj))
        print(plot_mx_agreement(mx_obj))
    }
